...
class StemSeparationEngine {
  ...
  Future<void> _ensureModelLoaded(StemModelSpec spec) async {
    ...
    final modelBytes = byteData.buffer
  ...
  Future<StemSeparationResult> separate({
    ...
    onProgress?.call(
        const StemSeparationProgress(phase: StemSeparationPhase.decoding));
    final tempDir = await getTemporaryDirectory();
    final pcmPath = p.join(tempDir.path,
        '${p.basenameWithoutExtension(sourceAudioPath)}_decoded.wav');
    await _pcmDecoder.decodeToPcm(sourceAudioPath, pcmPath);

    final pcm = await _wavCodec.read(pcmPath);
    if (pcm.sampleRate != spec.expectedSampleRate ||
        pcm.channelCount != spec.expectedChannels) {
      throw StateError(
    ...
    onProgress?.call(
        const StemSeparationProgress(phase: StemSeparationPhase.running));

    final channelStfts = channelSamples.map(_stft.forwardStft).toList();
    final totalStftFrames = channelStfts.first.real.length;
    final segmentCount = (totalStftFrames / StftConstants.timeFramesPerSegment)
        .ceil()
        .clamp(1, 1 << 30);
    ...
    for (int seg = 0; seg < segmentCount; seg++) {
      ...
      final segFrameCount = math.min(
          StftConstants.timeFramesPerSegment, totalStftFrames - segStart);

      final inputTensor = _buildInputTensor(
          channelStfts, segStart, segFrameCount, pcm.channelCount);
    ...
    onProgress
        ?.call(const StemSeparationProgress(phase: StemSeparationPhase.mixing));
    ...
    for (final stem in spec.producedStems) {
      ...
      for (int i = 0; i < frameCount; i++) {
        for (int c = 0; c < pcm.channelCount; c++) {
          interleaved[i * pcm.channelCount + c] =
              channelWaveforms[c][i].clamp(-1.0, 1.0);
        }
      }

      final outputPath =
          p.join(outputDir.path, 'stems', '${baseName}_${stem.name}.wav');
    ...
    onProgress?.call(const StemSeparationProgress(
        phase: StemSeparationPhase.complete, fractionComplete: 1.0));
    stopwatch.stop();

    return StemSeparationResult(
        stemFilePaths: stemPaths, processingTime: stopwatch.elapsed);
  }
    ...
    for (int t = 0; t < segFrameCount; t++) {
      ...
      for (int bin = 0; bin < StftConstants.keptFrequencyBins; bin++) {
        for (int c = 0; c < channelCount; c++) {
          ...
          final offset =
              (t * StftConstants.keptFrequencyBins + bin) * channelCount + c;
  ...
  Future<Map<StemType, Float32List>> _runInference(
    ...
    final inputTensor =
        OrtValueTensor.createTensorWithDataList(inputTensorData, shape);
    final runOptions = OrtRunOptions();

    final outputs =
        await session.runAsync(runOptions, {spec.inputTensorName: inputTensor});
  ...
  void _applyMaskAndAppend({
    ...
    for (int t = 0; t < segFrameCount; t++) {
      ...
      for (int c = 0; c < channelCount; c++) {
        final perStemReal = {
          for (final s in stems) s: Float64List(StftConstants.fullFrequencyBins)
        };
        final perStemImag = {
          for (final s in stems) s: Float64List(StftConstants.fullFrequencyBins)
        };
        ...
        for (int bin = 0; bin < StftConstants.fullFrequencyBins; bin++) {
          ...
          for (final stem in stems) {
            final estimate = maskEstimates[stem]![
                (t * StftConstants.keptFrequencyBins + bin) * channelCount + c];
          ...
          for (final stem in stems) {
            final mask =
                math.pow(estimatesAtBin[stem]!, StftConstants.wienerPower) /
                    sumOfPowers;
